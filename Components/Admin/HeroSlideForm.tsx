...
import { HeroSlide, HeroSlideFormData } from "@/lib/types/heroSlide";
...
export default function HeroSlideForm({ slide, isEditing = false }: HeroSlideFormProps) {
  ...
  const [formData, setFormData] = useState<HeroSlideFormData>({
    ...
    is_discount: slide?.is_discount ?? false,
    discount_percentage: slide?.discount_percentage || 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEditing ? `/api/hero-slides/${slide?.id}` : "/api/hero-slides";
      ...
      <div className="bg-white rounded-lg shadow p-4 sm:p-6 space-y-4 sm:space-y-6">
        ...
        <div>
          ...
          <input
            ...
            required
            className="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#660033] focus:border-transparent text-sm sm:text-base"
            placeholder="e.g., Discover Exquisite Flavors"
          />
        </div>
        ...
        <div>
          ...
          <ImageUploader
            images={formData.background_image ? [formData.background_image] : []}
            onChange={(images) => setFormData((prev) => ({ ...prev, background_image: images[0] || "" }))}
            maxImages={1}
          />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#660033] focus:border-transparent text-sm sm:text-base disabled:bg-gray-100 disabled:cursor-not-allowed"
              placeholder="0"
            />
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, is_discount: e.target.checked }))
              }
              className="w-4 h-4 text-[#660033] border-gray-300 rounded focus:ring-[#660033]"
            />
            <label htmlFor="is_discount" className="text-xs sm:text-sm text-gray-700">
              Show discount badge on this slide
            </label>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="is_active" className="text-xs sm:text-sm text-gray-700">
              Slide is active and visible on the website
            </label>
          </div>
        </div>
      </div>
